...
function aceptar(){
	if(document.getElementById("tituloAdv").innerHTML == `<i class="fas fa-check" style="color: #009688;" aria-hidden="true"></i>`){
		window.location.href = "http://localhost:8080/buscarUsuariosAdmin";
	}else if(document.getElementById("tituloAdv").innerHTML == "Bloquear usuario"){
		bloquear();
	} else if(document.getElementById("tituloAdv").innerHTML == "Desbloquear usuario"){
		desbloquear();
	}else{
		cancelar();
	}
