	...
	.then(function (response) {
		...
		if (jsonSucursal != null && jsonSucursal != [] && jsonSucursal != undefined && jsonSucursal.length != 0) {
			for (var i = 0; i < jsonSucursal.length; i++) {
				...
				nuevaOpcion.setAttribute("value", jsonSucursal[i].id);
				nuevaOpcion.appendChild(document.createTextNode(jsonSucursal[i].nombre));
				SelectSucursales.appendChild(nuevaOpcion);
			}
		} else {
			var nuevaOpcion = document.createElement("option");
			nuevaOpcion.setAttribute("value", "0");
			nuevaOpcion.appendChild(document.createTextNode("No hay sucursales registradas"));
			SelectSucursales.appendChild(nuevaOpcion);
		}
	}).catch(function (err) {
...
function BuscarLibro(Sucursal, ISBN, callback) {
	fetch("./registrarPromocion/libro/" + Sucursal + "/" + ISBN)
		.then(
			function (response) {
				if (response.status != 200)
					console.log("Ocurrió un error con el servicio: " + response.status);
...
function BuscarPromocion(Sucursal, ISBN, callback) {
	fetch("./registrarPromocion/" + Sucursal + "/" + ISBN + "/promocion")
		.then(
			function (response) {
				if (response.status != 200)
					console.log("Ocurrió un error con el servicio: " + response.status);
...
function libros(value) {
	fetch("/registrarPromocion/libros/"+value)
		.then(function (response) {
			//console.log(response.url);
			if (response.status != 200)
				console.log("Ocurrió un error con el servicio: " + response.status);
