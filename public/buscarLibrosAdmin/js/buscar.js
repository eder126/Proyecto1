...
function send(event) {
	const nombre = document.getElementById("buscarLibro");
...
function llenar(datos) {
	...
	datos.forEach((dato) => {
		...
		nuevo.innerHTML = `<li><a href="/perfilLibro/${dato.ISBN}" class="nombreSpan">${dato.nombre}</a>
		<a href="/editarLibro/${dato.ISBN}"><button type="button" class="btnAzul btnPequenno">Editar</button></a>
		<a href=""><button type="button" onclick='eliminar(${dato.ISBN});' class="btnRojo btnPequenno">Eliminar</button></a>
	</li>`;
...
function eliminar(isbn) {
	...
	fetch("/buscarLibrosAdmin/eliminarLibro/" + isbn).then(
		function (response) {
			if (response.status != 200) {
				console.log("Ocurrió un error con el servicio: " + response.status);
				error("Error", "Favor intentar luego", true, false);
