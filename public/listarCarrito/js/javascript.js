...
function aceptar(){
	...
	}else if(document.getElementById("tituloAdv").innerHTML == "Compra finalizada"){
		window.location.href = "http://localhost:8080/";
	}else if(document.getElementById("tituloAdv").innerHTML == "Cancelar compra"){
		eliminarCarrito();
	}else if(document.getElementById("tituloAdv").innerHTML == "Escoge un método de pago"){
		acceptoFinalizarPago();
	}else{
		cancelar();
	}
}
...
function error(titulo, msg, aceptar, cancelar){
	...
	if(titulo == "Compra finalizada"){ eliminarCarrito(); }
