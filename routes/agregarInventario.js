...
router.get("/", middleware.esLibreria, (req, res) => {
	res.sendFile(`${folder}/agregarInventario/index.html`);
});
...
router.get("/sucursales", middleware.esLibreria, (req, res) => {
	...
	sucursal.find({
		libreriaId: req.user.libreriaId
	}).sort({nombre: 1}).exec((err, encontrado) => {
		if (err) return res.json([]);
		console.log("Encontrado: " + typeof encontrado);
		console.log(encontrado);
		...
		if (encontrado.length != 0) {
			...
			encontrado.forEach(item => arr.push({id: item.id, nombre: item.nombre}));

			res.json({sucursales: arr});
